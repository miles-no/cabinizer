...
export function postProcessServiceOperations(
  service: Service,
  client: Client,
  ...
  return service.operations.map((operation) => {
    ...
    clone.parameters = clone.parameters.map((parameter) =>
      postProcessUnionTypes(parameter, client, useUnionTypes)
    );
    clone.results = clone.results.map((result) =>
      postProcessUnionTypes(result, client, useUnionTypes)
    );
    clone.imports.push(
